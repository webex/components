...
/**
 * Returns all of available cameras
 *
 * @returns {object[]} Available cameras
 */
...
export default function WebexVideoSettings({className, meetingID}) {
  const [devices, setDevices] = useState(null); // null means cameras are being enumerated
  ...
  useEffect(() => {
    getAvailableCameras()
      .then((cameras) => {
        global.testRendererAct(() => setDevices(cameras));
      });
  }, []);
